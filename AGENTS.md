...
- `cd packages/astrogwb-paper && uv run --group workflow snakemake --snakefile Snakefile --allowed-rules assemble_config run_mcmc plot_cosmological_parameters plot_modified_propagation experiments cosmological_parameters_chains modified_propagation_chains astrophysical_parameters_chains variable_injection_size_chains --dry-run --cores 1 experiments`:
