...
Usage::
    ...
    uv run python scripts/workflow.py mcmc configs/mcmc.batch.example.json --profile slurm --submit
...
def build_gen_configs_argv(extra: list[str] | None = None) -> list[str]:
    cmd = [
        ...
        "mcmc",
        "python",
        "scripts/generate_mcmc_configs.py",
        "--write-manifests",
    ]
    if extra:
        cmd.extend(extra)
...
def _validate(args: argparse.Namespace) -> None:
    ...
    if args.command == "catalog":
        _require_path(REPO_ROOT / CATALOG_SMK, kind="snakefile")
        return
    if args.command == "mcmc":
        _require_path(REPO_ROOT / MCMC_SMK, kind="snakefile")
        config = args.configfile
        if not config.is_absolute():
            config = REPO_ROOT / config
        _require_path(config, kind="configfile")
        _require_path(REPO_ROOT / profile_dir(args.profile), kind="profile")
        return
    if args.command == "paper":
        _require_path(REPO_ROOT / PAPER_SMK, kind="snakefile")
        return
