...
needed: one rule used to emit all 28 configs at once, so any edit invalidated
...
and catalog-checks all 28 runs without building anything, so a config typo fails
