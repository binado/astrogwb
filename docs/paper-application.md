...
`notebooks/fiducial_spectrum.py` and `notebooks/waveform_approximant_spectra.py` are marimo notebooks. Open them from the repository root with `uv run --extra notebook --group jupyter marimo edit notebooks/<name>.py`.
