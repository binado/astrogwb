...
import numpy as np
from _paper_style import combo_colors, use_paper_style
...
def plot_omega_and_sh(
    ...
    *,
    ...
    (line_sh,) = ax_sh.loglog(
        freq, sh, color=sh_color, linestyle=":", label=r"$S_h$"
    )
    ...
    ax_sh.set_xlabel(r"$f\ \mathrm{(Hz)}$", color=black)
    ax_sh.set_ylabel(r"$S_h(f)\ \mathrm{[Hz^{-1}]}$", color=black)
    ax_omega.set_ylabel(r"$\Omega_{\mathrm{GW}}(f)$", color=black)
    ax_sh.tick_params(axis="x", colors=black)
    ax_sh.tick_params(axis="y", colors=black)
    ax_omega.tick_params(axis="y", colors=black)
