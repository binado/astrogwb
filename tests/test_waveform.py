...
def test_bilby_backend_bns():
    pytest.importorskip("bilby")
    backend = BilbyWaveformBackend("IMRPhenomPV2_NRTidalv2", _GRID, "BNS")
    pol = backend.frequency_domain_polarizations(_BNS_PARAMS)
...
@pytest.mark.integration
def test_bilby_backend_bbh():
    pytest.importorskip("bilby")
    backend = BilbyWaveformBackend("IMRPhenomXP", _GRID, "BBH")
    pol = backend.frequency_domain_polarizations(_BBH_PARAMS)
    assert isinstance(pol, WaveformPolarizations)
    assert pol.hp.shape == pol.hc.shape


@pytest.mark.integration
def test_waveform_generator_end_to_end():
    pytest.importorskip("bilby")
    ...
    pol = gen.frequency_domain_polarizations(_BNS_PARAMS)
    assert isinstance(pol, WaveformPolarizations)
    assert pol.grid is _GRID
    assert pol.hp.shape == pol.hc.shape


@pytest.mark.integration
def test_bilby_backend_caches_generator():
