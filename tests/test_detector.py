...
def test_from_dict():
    psd = PowerSpectralDensity.from_noise_curve_dir("AplusDesign_psd.txt")
    det = Detector.from_dict(_H1_DICT, psd=psd)
...
def test_from_dict_with_string_psd():
    det = Detector.from_dict(_H1_DICT, psd="AplusDesign_psd.txt")

    assert isinstance(det.psd, PowerSpectralDensity)
    assert det.psd.file.name == "AplusDesign_psd.txt"


def test_from_dict_no_psd_raises():
    # dict has no default_noise_curve key and psd arg is None → KeyError
    incomplete = {k: v for k, v in _H1_DICT.items() if k != "name"}
