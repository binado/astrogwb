...
from .conftest import requires_gwfast
...
class TestGWFastComparison:
    """Compare our ORF implementation against GWFast's reference."""

    def _gwfast_orf(
        self, det1_name: str, det2_name: str, freqs: np.ndarray
    ) -> np.ndarray:
    ...
    def test_h1_l1_matches_gwfast(self, pair: tuple[str, str]) -> None:
        ...
        theirs = self._gwfast_orf(det1_name, det2_name, freqs)
        assert np.allclose(ours, theirs, atol=1e-4), (
            f"Max discrepancy for {det1_name}-{det2_name}: {np.max(np.abs(ours - theirs))}"
        )
