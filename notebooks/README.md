...
- **`waveform_approximant_spectra.py`** — compare TaylorF2, IMRPhenomXAS,
  ...
  tidal reference. It requires the `notebook` extra and the `jupyter` tooling
  group. It writes `outputs/figures/waveform_approximant_spectra.pdf`; there is
  no data cache, so every execution deterministically regenerates the draws
  from its fixed seed. Execute it from the repository root with:
