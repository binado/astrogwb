...
HDF5, so the notebooks reach for the paper package only for the cache.
...
`population_seed`, the band, and the fiducials — against its configuration
cell, and rebuilds on any
